...
impl FlashbotsConfig {
    ...
    fn create_clickhouse_infra(
        ...
        let task_executor = rbuilder_utils::tasks::Runtime::test();
        ...
        // Task to forward the abort to the task executor.
        let shutdown_runtime = task_executor.clone();
        tokio::spawn(async move {
            abort_token.cancelled().await;
            shutdown_runtime.graceful_shutdown();
        });
